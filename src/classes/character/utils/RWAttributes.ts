...
export namespace RWCAttributes {
  export const getRarity = (id: number) => {
    ...
    const characterRarity = characterConfig?.rarity;
  ...
  export const getElement = (id: number) => {
    const characterConfig = RWUtils.getConfig(id);
    const elementConfig = characterConfig
      ? RWUtils.getElementConfig(characterConfig.element - 1)
      : undefined;

    const characterElement: ICharacterElement = {
      id: characterConfig?.element ?? -1,
      image: elementConfig
        ? getImageUrl(characterElements[elementConfig?.image])
        : undefined,
  ...
  export const getBond = (id: number) => {
    ...
    Object.keys(favourExpData).forEach((_, index) => {
      const data = favourExpData[index];
      if (!data || data.id === 10) return;

      const presentData = favourPresentData[id];
